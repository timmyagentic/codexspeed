...
export function MetricSelector({ value, onChange }: MetricSelectorProps) {
  function selectFromKey(event: KeyboardEvent<HTMLButtonElement>, index: number) {
    let nextIndex: number | null = null;
    if (event.key === "ArrowRight") nextIndex = (index + 1) % METRIC_KEYS.length;
    if (event.key === "ArrowLeft") nextIndex = (index - 1 + METRIC_KEYS.length) % METRIC_KEYS.length;
  ...
  return (
    <div className="metric-selector" role="tablist" aria-label="Benchmark metric">
      ...
      <span className="metric-direction" aria-live="polite">
        {METRICS[value].unit} ({METRICS[value].higherIsBetter ? "higher" : "lower"} is better)
      </span>
    </div>
  );
}
